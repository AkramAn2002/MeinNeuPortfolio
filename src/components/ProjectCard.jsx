...
const LABEL_PADDING = 16; // px d'espace supplémentaire entre labels

// Mesure la largeur réelle d'un texte dans le DOM
function measureTextWidth(text, fontSize = "12px", fontWeight = "600") {
...
// Anti-collision basé sur la largeur réelle des labels
function resolveCollisions(labels) {
  if (labels.length < 2) return labels;

  // Enrichir chaque label avec sa demi-largeur réelle
  const enriched = labels.map((lab, i) => ({
    ...lab,
    origIdx: i,
    halfW: measureTextWidth(lab.text) / 2 + LABEL_PADDING,
  }));

  // Trier par x
  enriched.sort((a, b) => a.x - b.x);

  // Plusieurs passes de séparation
  for (let pass = 0; pass < 5; pass++) {
    for (let i = 1; i < enriched.length; i++) {
      const prev = enriched[i - 1];
      const curr = enriched[i];
      // Distance minimale = demi-largeur de chacun
      const minGap = prev.halfW + curr.halfW;
      const gap = curr.x - prev.x;
      if (gap < minGap) {
        const shift = (minGap - gap) / 2;
        enriched[i - 1] = { ...prev, x: prev.x - shift };
        enriched[i] = { ...curr, x: curr.x + shift };
      }
    }
    // Passe inverse
    for (let i = enriched.length - 2; i >= 0; i--) {
      ...
      const minGap = curr.halfW + next.halfW;
      const gap = next.x - curr.x;
      if (gap < minGap) {
        const shift = (minGap - gap) / 2;
        enriched[i] = { ...curr, x: curr.x - shift };
        enriched[i + 1] = { ...next, x: next.x + shift };
      }
    }
  }
...
const ProjectCard3D = ({ project, isActive, onClick, carouselOffset }) => {
  ...
  const compute = useCallback(() => {
    ...
    const dots = project.infoPoints.map((p) => ({
    ...
    const rawLabels = dots.map((d) => ({
      x: d.x,
      y: LABEL_MARGIN - 4,
      text: d.text,
    }));

    const resolvedLabels = resolveCollisions(rawLabels);

    const lines = dots.map((d, i) => ({
      dotX: d.x,
      dotY: d.dotY,
      labelX: resolvedLabels[i].x,
      topY: d.topY,
    }));
              ...
              return (
                <g key={i}>
                  ...
                  <line
                    ...
                    style={{
                      transition: `stroke-dashoffset 0.55s cubic-bezier(0.4,0,0.2,1) ${i * 0.13}s`,
                    }}
                    ref={(el) => {
                      if (el)
      ...
      <AnimatePresence>
        {isActive &&
          ...
          labels.map((lab, i) => (
            <motion.span
              key={i}
              className="absolute pointer-events-none z-50 text-white/80 text-[12px] font-semibold tracking-wide whitespace-nowrap"
              style={{
                left: lab.x,
                top: lab.y - 20,
